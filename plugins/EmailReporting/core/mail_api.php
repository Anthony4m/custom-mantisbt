...
//					trigger_error( ERROR_CUSTOM_FIELD_INVALID_VALUE, ERROR );
		...
		$this->auto_reply_mail($p_email[ 'From_parsed' ][ 'email' ],$t_bug_id);
		// echo json_encode($p_email[ 'From_parsed' ][ 'email' ]);
	...
	private function auto_reply_mail($p_email,$p_bug_id){
		$t_subject = email_build_subject($p_bug_id);
		// $t_date = date(config_date('normal_date_format'));
		// $sender_id = auth_get_current_user_id();
		// $t_sender = user_get_name( $t_sender_id );
		// $t_sender_email = user_get_email($sender_id);
		$f_body	= bug_format_summary( $p_bug_id, SUMMARY_CAPTION );
		$t_header = "\n" . lang_get( 'on_date' );
		$t_contents = $t_header .  " \n\n$f_body";
		$t_contents .= " Thank you for reporting your issue our"  . "\n" . 
		"team is working tirelessly on it " . "\n" 
		. "this is your issue id " . $p_bug_id ;
		email_store( $p_email, $t_subject, $t_contents, null, true );
		// email_store( $p_email, $t_subject, $t_contents);
	}
	# --------------------
